...
THIS IS A FORK FROM https://gist.github.com/Asbra/484165c4dd171e58275a1a0fb83e6978
MOST OF THIS CODE IS WRITTEN BY https://github.com/Asbra/
...
gamedirs = [r'C:\Users\gabri\League of Legends',
            r'C:\Users\gabri\League of Legends']
...
while True:
    if championIdx >= len(championsBan):
        championIdx = 0

    if championIdx2 >= len(championsPrio):
        championIdx2 = 0
    ...
    # Pick/lock champion
        ...
        for groups in cs['actions']:
            for action in groups:
                if action['actorCellId'] != actorCellId:
                    continue

                if action['isInProgress']:
                    if action['type'] == "ban" and cs['timer']['phase'] != 'PLANNING':
                        sleep(2)
                        print('ban')
                        try:
                            championId = championsBan[championIdx]
                        ...
                        print('Ban', championName, '(%d)' % championId, '..')
                    ...
                    if action['type'] == "pick":
                        ...
                        championName = champions[str(championId)]
                        print('Pick', championName, '(%d)' % championId, '..')
